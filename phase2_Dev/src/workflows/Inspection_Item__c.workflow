...
<Workflow xmlns="http://soap.sforce.com/2006/04/metadata">
    ...
    <rules>
        <fullName>Create Submission History</fullName>
        <actions>
            <name>Create_History_Record_flag</name>
            <type>FieldUpdate</type>
        </actions>
        <active>true</active>
        <description>When providers send back a noncompliant statement and it is rejected by a licensor, providers can delete previous response and write new response. Previous responses should be kept as part of the record of what providers have submitted.</description>
        <formula>AND( OR( AND( NOT( ISBLANK( Correction_Plan__c )), ISCHANGED (Correction_Plan__c ), NOT(Save_for_Later__c) ),  AND ( ISCHANGED(Save_for_Later__c), NOT(Save_for_Later__c), NOT( ISBLANK( Correction_Plan__c )))) /*, NOT(ISBLANK(PRIORVALUE( Date_Corrective_Actions_Received__c )))*/ )</formula>
